...
--- Lightweight check: returns true if any hostile unit exists within range.
--- No table allocation, no Unit:New wrappers — raw entity cache + distance check.
function Unit:hasHostileNearby(range)
  ...
  for _, e in ipairs(entities) do
    local cls = e.class
    if cls ~= "Unit" and cls ~= "Player" then goto skip_h end
    local eu = e.unit
    if not eu then goto skip_h end
    if eu.is_dead then goto skip_h end
    if e.guid == self.Guid then goto skip_h end

    local ep = e.position
    if not ep then goto skip_h end
    local d = game.distance(sp.x, sp.y, sp.z, ep.x, ep.y, ep.z)
    if d <= range then
      local ok, enemy = pcall(game.unit_is_enemy, self.obj_ptr, e.obj_ptr)
      if ok and enemy then return true end
    end
    ::skip_h::
  end
  return false
