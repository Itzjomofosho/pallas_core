...
local options = {
  ...
  Widgets = {
    ...
    { type = "checkbox", uid = "SVAutoAspect",
      text = "Auto Aspect switching",     default = true },
...
local function SurvivalCombat()
  ...
  -- Aspect switching — Hawk when hostiles nearby (me or party), Pack otherwise
  if PallasSettings.SVAutoAspect ~= false then
    -- Throttled to avoid per-tick entity scans
    local now = os.clock()
    if not _sv_hawk_next or now >= _sv_hawk_next then
      _sv_hawk_need = false
      _sv_hawk_next = now + 0.5

      if #Combat.Targets > 0 then
        _sv_hawk_need = true
      elseif Me:hasHostileNearby(20) then
        _sv_hawk_need = true
      else
        local friends = Heal and Heal.Friends and Heal.Friends.All or {}
        for _, friend in ipairs(friends) do
          if friend.Guid ~= Me.Guid and Me:GetDistance(friend) <= 40 then
            if friend:hasHostileNearby(20) then
              _sv_hawk_need = true
              break
            end
          end
